...
class DbHelper : SQLiteOpenHelper {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        var createOrderTable =
            "CREATE TABLE $TB_ORDER ($COL_ORDERNUMBER INTEGER PRIMARY KEY AUTOINCREMENT, $COL_ORDERDATE VARCHAR(10), $COL_REQUIREDDATE VARCHAR(10), $COL_SHIPPEDDATE VARCHAR(10), $COL_STATUS TEXT, $COL_COMMENTS TEXT, $COL_CUSTOMERNUMBER INTEGER)"
        var createOrderDetailsTable =
            "CREATE TABLE $TB_ORDERDETAILS($COL_ORDERNUMBER INTEGER PRIMARY KEY AUTOINCREMENT, $COL_PRODUCTCODE INTEGER, $COL_QUANTITYORDERED VARCHAR(10),$COL_PRICEEACH VARCHAR(10), $COL_ORDERLINENUMBER INTEGER)"
    ...
    fun deleteData(detail: OrderDetails): Boolean {
        ...
        return db.delete(
            TB_ORDERDETAILS,
            "$COL_ORDERNUMBER = $detail.orderNumber $COL_PRODUCTCODE = ${detail.productCode}",
            null
        ) > 0
